...
export function generateQuestionnaireReportPdf(data: QReportData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(11).font("Helvetica").fillColor(scoreColor(overallScore))
    ...
    doc.fontSize(7.5).font("Helvetica-Bold").fillColor(C.amber)
       .text("AVISO: ", MARGIN + 10, y + 4, { continued: true });
    doc.font("Helvetica").fillColor(C.amber)
       .text(
    ...
    for (const m of measureScores) {
      ...
      if (m.score === null) {
        ...
        // Score number
        doc.fontSize(7.5).font("Helvetica-Bold").fillColor(scoreColor(m.score))
           .text(String(m.score), MARGIN + BAR_LABEL_W + BAR_W + 4, y + 1, { width: 28 });
        // Gap count
        if (m.gapCount > 0) {
          doc.fontSize(6.5).font("Helvetica").fillColor(C.danger)
